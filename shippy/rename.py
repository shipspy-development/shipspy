...
def fix_times(ds, instrument):
    ...
    if instrument in ['calitoo', 'dusttrak']:
        timestamps = ds.time*np.timedelta64(1, 's').astype('timedelta64[ns]') + np.datetime64('1970-01-01T00:00:00').astype('datetime64[ns]')
        ds["time"] = timestamps
    elif instrument == 'microtops':
        ...
        for d,t in zip(dates, times):
            tstamp = np.array([np.datetime64(dt.strptime(d + ' ' +t, '%d:%m:%Y %H:%M:%S'),'s')])
...
def get_units_old(ds, instrument):
    ...
    if instrument == 'calitoo':
        ...
        for v in list(ds.keys()):
            unit = ds[v].long_name[ds[v].long_name.find('[')+1:ds[v].long_name.find(']')]
            unit = ds[v].long_name[ds[v].long_name.find('[')+1:ds[v].long_name.find(']')]
            if not unit.find(',') == -1:
...
def add_extras(ds, instrument):
    ...
    """
    if instrument == 'ctd':
        ds = ds.assign_coords(alt = ('DEPTH', -ds.DEPTH.values)) #assign altitude as auxiliary coordinate
...
def put_attrs(ds, variable_dict, old_attrs):
    ...
    """
    varname_dic = {key: variable_dict[key]["varname"] for key in list(variable_dict.keys())}
