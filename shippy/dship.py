...
def circ_mean(da):
    return xr.apply_ufunc(CircMean, da, input_core_dims=[['time']], vectorize=True, dask="parallelized")
...
def min_means(ds, var):
    ...
    if var in list(['wdir', 'ship_heading']):
        var_mean = circ_mean(ds[var].where(ds[var] > 0, other = np.nan).dropna('time').resample(time = 'min')).rename({'__resample_dim__': 'time'}).to_dataset()
    else:
...
def var_ordering(ship):
    if ship == 'Merian':
        ordering = ['t_air', 'p_air', 'wspd', 'wdir', 'rh', 'lwr', 'swr', 'sea_floor_depth', 'sst_2m', 'sst_7m', 'conductivity', 'salinity', 'chlorophyll_a', 'current_speed', 'current_dir','wave_period', 'wave_length', 'wave_height', 'wave_dir', 'ship_speed', 'ship_heading', 'ship_heave', 'ship_heave_std', 'ship_pitch', 'ship_pitch_std', 'ship_roll', 'ship_roll_std']
    elif ship == 'Sonne':
...
def run(args):
    ...
    varname_dict = {key: rename_dict[key]["varname"] for key in list(rename_dict.keys())}
    varname_swap = {v: k for k, v in varname_dict.items()}
    std_dict = {'ship_heave': {'varname' : 'ship_heave_std', 'long_name': 'standard deviation of ship heave', 'units': 'm', 'instrument': 'Seapath (DShip)', 'cell_method': 'minutely standard deviation'},
               'ship_roll': {'varname' : 'ship_roll_std', 'long_name': 'standard deviation of ship roll', 'units': 'degree', 'instrument': 'Seapath (DShip)', 'cell_method': 'minutely standard deviation'},
               'ship_pitch': {'varname' : 'ship_pitch_std', 'long_name': 'standard deviation of ship pitch', 'units': 'degree', 'instrument': 'Seapath (DShip)', 'cell_method': 'minutely standard deviation'}}
    ...
    if ship_name == 'Merian':
        dship_data.lat, dship_data.lon = dship_data.lat.apply(format_loc), dship_data.lon.apply(format_loc)
    ...
    timestamps = np.datetime64('1970-01-01T00:00:00') + secs_since_1970*np.timedelta64(1,'s')
