...
def _run(args):
    ...
    sct_header = np.argwhere(complete_period.dates.values == 'section_number;section_cuts').flatten()[0]
    cutout_header = np.argwhere(complete_period.dates.values == 'cutout_start;cutout_end').flatten()[0]
    cutouts = pd.read_csv(secfilename, header = cutout_header, delimiter = ';')[:sct_header-cutout_header-1]
    for cut_start, cut_end in zip(cutouts.cutout_start.values, cutouts.cutout_end.values):
    ...
    section_coords = np.zeros(len(ds.sel({dim_name: slice(None,np.datetime64(sec_cuts[0])-np.timedelta64(1,'s'))})[dim_name]))
    for i,n in zip(np.arange(len(sec_cuts[-1])), sec_nums[:-1]):
        sec = n*np.ones(len(ds.sel({dim_name: slice(np.datetime64(sec_cuts[i]),np.datetime64(sec_cuts[i+1])-np.timedelta64(1,'s'))})[dim_name]))
        section_coords = np.append(section_coords, sec)
    sec = sec_nums[-1]*np.ones(len(ds.sel({dim_name: slice(np.datetime64(sec_cuts[-1]),None)})[dim_name]))
    section_coords = np.append(section_coords, sec)
    ...
    ds.to_netcdf(outfilename, encoding={dim_name: {'dtype': '<i4', 'units': f'seconds since {campaign_start}'}})