...
#                .dropna("time")
    ...
    elif var in ["quality_position", "quality_meteorology", "quality_aerosols", "quality_tracegases", "quality_housekeeping", "precip_acc"]:
...
def quality_flag(ds, vars, flag_dict, deltat):
    ...
    for v in vars:
        ...
        too_noisy = (np.argwhere(ds[v].rolling(time = deltat, center = True).var().values > var)).flatten()
...
def get_input_data(header_file, indir):
    ...
    datetime = np.datetime64("1970-01-01T00:00") + data["timestamp"].values*np.timedelta64(1, "s")
...
def erasing(ds, v, eraser):
    if v in ["uv_bc1", "blue_bc1", "green_bc1", "red_bc1", "ir_bc1", "opcr2_pm10", "opcr2_pm2p5", "opcr2_pm1", "number", "diameter", "ldsa", "co2_conc", "co_conc", "ch4_conc", "so2_conc"]:
...
def run(args):
    ...
    try:
        ...
        aero_vars = ["number_conc", "pm10", "pm2p5", "pm1", "bc_375nm", "bc_470nm", "bc_528nm", "bc_625nm", "bc_880nm"]
